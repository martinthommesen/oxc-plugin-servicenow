...
export function assertFix(
  ...
  const messages = lint(code, rule, options);
  const withFix = messages.filter((message) => message.fixedSource !== undefined);
  assert.ok(
    withFix.length >= 1,
    `Expected a fix for ${rule}, got ${messages.length} message(s) and no fixedSource:\n${messages
      .map((message) => `  - ${message.messageId ?? "?"} ${message.message}`)
      .join("\n")}`,
  );
  assert.equal(withFix[0]!.fixedSource, expectedOutput);
}

export function assertSuggestion(
  code: string,
  rule: RuleName,
  desc: RegExp | string,
  expectedOutput: string,
  ...
  const messages = lint(code, rule, options);
  const matches = (text: string) => (typeof desc === "string" ? text === desc : desc.test(text));
  for (const message of messages) {
    const found = message.suggestions?.find((suggestion) => matches(suggestion.desc));
    if (found) {
      assert.equal(found.fixedSource, expectedOutput);
      return;
    }
  }
  const listed = messages
    .flatMap((message) => message.suggestions ?? [])
    .map((suggestion) => `  - ${suggestion.desc}`)
    .join("\n");
  assert.fail(
    `Expected a suggestion matching ${String(desc)} for ${rule}, got:\n${listed || "  (none)"}`,
  );
}
