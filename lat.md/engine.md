...
Separately, `AUSTRALIA_RULE_REVIEWS` in `src/catalog-metadata.ts` gives all 51 rules a release review — `reviewed` with bases, `invariant` with a rationale, or `not-applicable` on the Fluent SDK axis. Zurich is the legacy baseline and passes everything.
