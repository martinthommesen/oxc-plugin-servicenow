...
function latest(evidence: readonly RuleEvidenceRecord[]): string {
  return evidence.reduce((max, item) => (item.verifiedAt > max ? item.verifiedAt : max), VERIFIED);
}
...
export const ruleDocMetadata: Record<string, RuleDocMetadata> = {
  "no-hardcoded-sysid": meta(classic(CLASSIC_SURFACES), [
    ev(SN_FLUENT_CONSTRUCTS, "Named Fluent Now.ID keys are the supported portable identity, not raw sys_id literals.", "declaration-snapshot"),
    ev("tests/rules/no-hardcoded-sysid.test.ts", "Literal 32-hex strings report; settings and option allow-lists suppress.", "fixture"),
  ], {
  ...
  "no-promise": meta(engine(ES5_MODES), [
    ev(SN_JS_FEATURES, "Promises are unsupported in Compatibility and ES5 Standards modes.", "declaration-snapshot"),
    ev("tests/rules/no-promise.test.ts", "Platform Promise identifiers report; local bindings stay silent.", "fixture"),
  ], {
  ...
  "no-async-await": meta(engine(ES5_MODES), [
    ev(SN_JS_FEATURES, "async/await is unsupported in Compatibility and ES5 Standards modes.", "declaration-snapshot"),
    ev("tests/rules/no-async-await.test.ts", "async functions and await expressions report in ES5 mode.", "fixture"),
  ], {
  ...
  "no-bigint": meta(engine(ES5_MODES), [
    ev(SN_JS_FEATURES, "BigInt is unsupported in Compatibility and ES5 Standards modes.", "declaration-snapshot"),
    ev("tests/rules/no-bigint.test.ts", "BigInt literals and the platform BigInt identifier report.", "fixture"),
  ], {
  ...
  "prefer-glideaggregate": meta(classic(SERVER_SURFACES), [
    ev(SN_GR, "GlideAggregate is the documented API for count and group queries.", "declaration-snapshot"),
    ev("tests/rules/prefer-glideaggregate.test.ts", "Iterate-to-count loops report; if (gr.next()) stays silent.", "fixture"),
  ], {
  ...
  "no-client-gliderecord": meta(classic(CLIENT_SURFACES), [
    ev(SN_GR, "GlideRecord is a server API and is not a client-side record cursor.", "declaration-snapshot"),
    ev("tests/integration/profiles/invalid/client-gliderecord.client.js", "Recommended Oxlint and ESLint flag GlideRecord in client files.", "integration-test"),
  ], {
  ...
  "no-gs-now": meta(classic(CLASSIC_SURFACES), [
    ev(SN_GDT, "gs.now() and gs.nowDateTime() return display strings, not GlideDateTime objects.", "declaration-snapshot"),
    ev("tests/integration/fixtures/bad-business-rule.br.js", "Host fixtures report gs.now on Business Rule files.", "integration-test"),
  ], {
  ...
  "require-query-before-next": meta(classic(SERVER_SURFACES), [
    ev(SN_GR, "next() reads the current cursor row after query() or get() executes the query.", "declaration-snapshot"),
    ev("tests/integration/profiles/invalid/missing-query.br.js", "Oxlint and ESLint report next() without a preceding query on every path.", "integration-test"),
    ev("tests/rules/stateful-lifecycle.test.ts", "Aliases, sibling reassignment, and completion-aware paths are unit-tested.", "fixture"),
  ], {
    falsePositives: [],
  ...
  "validate-gliderecord-calls": meta(classic(SERVER_SURFACES), [
    ev(SN_GR_GLOBAL, "Deprecated compatibility rule. Prefer require-query-before-next for query lifecycle.", "declaration-snapshot"),
    ev("src/catalog.ts", "The rule remains exported and off by default.", "declaration-snapshot"),
  ], {
  ...
  "no-br-current-update": meta(classic(["business-rule"]), [
    ev(SN_BR, "Business Rules should not call current.update() because the engine already writes the row.", "declaration-snapshot"),
    ev("tests/integration/fixtures/bad-business-rule.br.js", "Host fixtures report current.update on Business Rule files.", "integration-test"),
  ], {
  ...
  "no-hardcoded-table-names": meta(classic(SERVER_SURFACES), [
    ev(SN_GR, "Table names passed to GlideRecord constructors are string identities that do not rename safely.", "declaration-snapshot"),
    ev("tests/rules/glide-and-engine.test.ts", "Literal tables report; named constants and allow-lists stay silent.", "fixture"),
  ], {
  ...
  "fluent-proper-imports": meta(fluent(), [
    ev(SN_FLUENT, "Fluent factories are imported from the documented @servicenow/sdk modules.", "declaration-snapshot"),
    ev("tests/integration/fixtures/bad-fluent.now.ts", "Host fixtures report factories imported from the wrong module.", "integration-test"),
  ], {
  ...
  "fluent-directives": meta(fluent(), [
    ev(SN_FLUENT, "Fluent ignore directives are line- and file-scoped comments recognized by the SDK toolchain.", "declaration-snapshot"),
    ev("tests/integration/profiles/invalid/dangling-fluent-ignore.now.ts", "A trailing @fluent-ignore without a following statement reports.", "integration-test"),
  ], {
    falsePositives: [],
  ...
  "prefer-now-include": meta(fluent(), [
    ev(SN_FLUENT_CONSTRUCTS, "Now.include() loads script and markup files so Fluent metadata stays declarative.", "declaration-snapshot"),
    ev("src/catalog.ts", "Catalog examples cover large inline script versus Now.include.", "fixture"),
  ], {
  ...
  "require-fluent-id": meta(fluent(), [
    ev(SN_FLUENT_CONSTRUCTS, "Factories whose manifest marks $id as required must declare Now.ID or an equivalent id.", "declaration-snapshot"),
    ev("tests/integration/profiles/invalid/fluent-alias-missing-id.now.ts", "Aliased factory imports still require $id under recommended.", "integration-test"),
  ], {
  ...
  "fluent-naming-convention": meta(fluent(), [
    ev(SN_FLUENT, "Fluent file stems and Now.ID keys should stay stable kebab-case or snake_case identifiers.", "declaration-snapshot"),
    ev("src/catalog.ts", "Catalog examples cover PascalCase files and kebab-case corrections.", "fixture"),
  ], {
  ...
  "no-complex-fluent-logic": meta(fluent(), [
    ev(SN_FLUENT, "Fluent .now.ts files declare metadata; runtime loops belong in src/server.", "declaration-snapshot"),
    ev("src/catalog.ts", "Catalog examples cover a runtime loop versus declarative metadata.", "fixture"),
  ], {
  ...
  "no-at-method": meta(engine(ES5_MODES), [
    ev(SN_JS_FEATURES, "Array.prototype.at is unsupported in Compatibility and ES5 Standards modes.", "declaration-snapshot"),
    ev("src/catalog.ts", "Catalog examples cover array.at versus bracket access.", "fixture"),
  ], {
  ...
  "no-packages-calls": meta(classic(CLASSIC_SURFACES, ALL_MODES), [
    ev(SN_JS_FEATURES, "Packages.* Java interop is not a supported ServiceNow JavaScript API.", "declaration-snapshot"),
    ev("src/catalog.ts", "Catalog examples cover Packages.java versus local bindings named Packages.", "fixture"),
  ], {
  ...
  "no-weak-references": meta(classic(CLASSIC_SURFACES, ALL_MODES), [
    ev(SN_JS_FEATURES, "WeakRef and FinalizationRegistry are unsupported in instance JavaScript modes.", "declaration-snapshot"),
    ev("src/catalog.ts", "Catalog examples cover WeakRef construction.", "fixture"),
  ], {
  ...
  "no-weak-collections": meta(engine(ES5_MODES), [
    ev(SN_JS_FEATURES, "WeakMap and WeakSet are unsupported in Compatibility and ES5 Standards modes.", "declaration-snapshot"),
    ev("src/catalog.ts", "Catalog examples cover WeakMap construction in ES5 mode.", "fixture"),
  ], {
    falsePositives: ["Local bindings that reuse those names."],
    falseNegatives: ["Dynamic construction through unknown identifiers."],
  ...
  "no-typed-arrays": meta(engine(ES5_MODES), [
    ev(SN_JS_FEATURES, "Typed arrays are unsupported in Compatibility and ES5 Standards modes.", "declaration-snapshot"),
    ev("src/catalog.ts", "Catalog examples cover Uint8Array construction.", "fixture"),
  ], {
  ...
  "no-proxy": meta(engine(ES5_MODES), [
    ev(SN_JS_FEATURES, "Proxy is unsupported in Compatibility and ES5 Standards modes.", "declaration-snapshot"),
    ev("src/catalog.ts", "Catalog examples cover new Proxy versus a local binding.", "fixture"),
  ], {
  ...
  "no-unsupported-syntax": meta(engine(ES5_MODES), [
    ev(SN_JS_FEATURES, "Several ES2015+ syntactic forms are unsupported in Compatibility and ES5 Standards modes.", "declaration-snapshot"),
    ev("tests/integration/profiles/invalid/es5-promise.server.js", "classic-es5 Oxlint flags unsupported syntax on the ES2021 fixture.", "integration-test"),
  ], {
    falsePositives: ["Files whose javascriptMode is unknown or es2021."],
  ...
  "no-delete-multiple-with-windowing": meta(classic(SERVER_SURFACES), [
    ev(SN_GR, "setLimit and chooseWindow do not limit deleteMultiple(); the call deletes every matching row.", "declaration-snapshot"),
    ev("tests/integration/profiles/invalid/windowed-delete.br.js", "Recommended hosts report windowed deleteMultiple.", "integration-test"),
  ], {
    falsePositives: [],
  ...
  "require-callback-for-getreference": meta(classic(CLIENT_SURFACES), [
    ev(SN_FORM, "g_form.getReference without a callback is a synchronous server request.", "declaration-snapshot"),
    ev("tests/integration/profiles/invalid/sync-getreference.client.js", "Recommended hosts report the one-argument form.", "integration-test"),
  ], {
  ...
  "require-glideajax-sysparm-name": meta(classic(CLIENT_SURFACES), [
    ev(SN_AJAX, "GlideAjax requires a non-empty sysparm_name before getXML, getXMLAnswer, or getXMLWait.", "declaration-snapshot"),
    ev("tests/integration/profiles/invalid/glideajax-empty-sysparm.client.js", "Empty or missing sysparm_name values report on the client host fixtures.", "integration-test"),
  ], {
    falsePositives: [],
  ...
  "validate-glideaggregate-calls": meta(classic(SERVER_SURFACES), [
    ev(SN_GR, "getAggregate reads a tuple that addAggregate registered before the open query.", "declaration-snapshot"),
    ev("tests/integration/profiles/invalid/aggregate-type-only-field.br.js", "Type-only COUNT does not satisfy a field-specific getAggregate.", "integration-test"),
  ], {
    falsePositives: [],
  ...
  "no-now-id-as-reference": meta(fluent(), [
    ev(SN_FLUENT_CONSTRUCTS, "Now.ID is a metadata identity, not an in-app record reference.", "declaration-snapshot"),
    ev("tests/integration/profiles/invalid/now-id-ref.now.ts", "Recommended hosts report Now.ID used as a reference field.", "integration-test"),
  ], {
  ...
  "no-glideajax-getanswer": meta(classic(CLIENT_SURFACES), [
    ev(SN_GLIDEAJAX, "getAnswer belongs to the synchronous getXMLWait pattern.", "declaration-snapshot"),
    ev("tests/integration/profiles/invalid/glideajax-getanswer.client.js", "Recommended hosts report getAnswer on proven GlideAjax objects.", "integration-test"),
  ], {
    falsePositives: [],
  ...
  "no-duplicate-fluent-id": meta(fluent(), [
    ev(SN_FLUENT_CONSTRUCTS, "Now.ID keys must be unique in a file so keys.ts can track records.", "declaration-snapshot"),
    ev("tests/integration/profiles/invalid/duplicate-id.now.ts", "Recommended hosts report duplicate Now.ID keys.", "integration-test"),
  ], {
    falsePositives: [],
  ...
  "no-glideelement-in-collection": meta(classic(SERVER_SURFACES), [
    ev(SN_GR, "A GlideElement from a cursor follows the cursor; collections must store extracted values.", "declaration-snapshot"),
    ev("tests/integration/profiles/invalid/glideelement-push.br.js", "Recommended hosts report pushing a cursor field into an array.", "integration-test"),
  ], {
    falsePositives: [],
  ...
  "no-gliderecord-query-modifier-after-query": meta(classic(SERVER_SURFACES), [
    ev(SN_GR_GLOBAL, "Query modifiers after query() or get() do not change the open cursor.", "declaration-snapshot"),
    ev("tests/integration/profiles/invalid/late-modifier.br.js", "Recommended hosts report addQuery after query before next.", "integration-test"),
  ], {
    falsePositives: [],
  ...
  "require-business-rule-wrapper": meta(
    ...
    [
      ev(SN_BR, "Full-script Business Rules use the executeRule(current, previous) IIFE so top-level bindings do not leak.", "declaration-snapshot"),
      ev("tests/integration/profiles/invalid/unwrapped.br.js", "The wrapper rule reports only when businessRuleSourceFormat is full-script.", "integration-test"),
    ],
    {
  ...
  "no-display-value-date-comparison": meta(classic(SERVER_SURFACES), [
    ev(SN_GDT, "GlideDateTime.getDisplayValue() follows the session format and is not a chronological sort key.", "declaration-snapshot"),
    ev("src/catalog.ts", "Catalog examples cover display-value comparison versus getNumericValue.", "fixture"),
  ], {
  ...
  "no-unfiltered-gliderecord-bulk-operation": meta(classic(SERVER_SURFACES), [
    ev(SN_GR, "updateMultiple and deleteMultiple apply to every row that matches the query filters.", "declaration-snapshot"),
    ev("tests/integration/profiles/invalid/empty-addquery-bulk.br.js", "Empty or missing addQuery arguments do not count as filters.", "integration-test"),
  ], {
    falsePositives: [],
  ...
  "no-gliderecord-query-in-loop": meta(classic(SERVER_SURFACES), [
    ev(SN_GR, "query or get inside a next() loop is an N+1 pattern on the GlideRecord cursor.", "declaration-snapshot"),
    ev("tests/integration/profiles/invalid/nested-cursor-query.br.js", "Strict hosts report a nested query inside a proven cursor loop.", "integration-test"),
    ev("tests/integration/profiles/valid/custom-iterator-loop.br.js", "Custom iterators with next() do not establish cursor depth.", "integration-test"),
  ], {
    falsePositives: [],
  ...
  "prefer-setnocount-with-choosewindow": meta(classic(SERVER_SURFACES), [
    ev(SN_GR, "query() after chooseWindow() runs COUNT(*) unless setNoCount() or setLimit() skips it.", "declaration-snapshot"),
    ev("tests/integration/profiles/invalid/setnocount-second-query.br.js", "A later query epoch is not justified by an earlier getRowCount().", "integration-test"),
  ], {
  ...
  "no-system-query-bypass": meta(classic(SERVER_SURFACES), [
    ev(SN_GR, "addSystemQuery and related methods bypass query ACLs and need review.", "declaration-snapshot"),
    ev("tests/integration/profiles/invalid/system-query.br.js", "The security profile reports documented ACL-bypass methods.", "integration-test"),
  ], {
  ...
  "no-sync-glideajax": meta(classic(CLIENT_SURFACES), [
    ev(SN_GLIDEAJAX, "getXMLWait is a synchronous browser request.", "declaration-snapshot"),
    ev("src/catalog.ts", "Catalog examples cover getXMLWait versus getXMLAnswer.", "fixture"),
  ], {
    falsePositives: [],
  ...
  "no-async-iterators": meta(engine(ALL_MODES), [
    ev(SN_JS_FEATURES, "for await...of and async generators are disallowed in every instance JavaScript mode.", "declaration-snapshot"),
    ev("tests/integration/profiles/invalid/es2021-async-iter.server.js", "es2021 Oxlint still flags async iteration.", "integration-test"),
  ], {
    falsePositives: [],
