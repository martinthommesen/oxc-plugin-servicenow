...
import { findGlideAjaxParamIssues } from "../analysis/internal.js";
...
export const requireGlideajaxSysparmName = defineRule({
  meta: {
    ...
    messages: {
      missingName:
        '`{{name}}` starts a GlideAjax request without `addParam("sysparm_name", ...)`. Call `addParam` with a non-empty Script Include method name before the request.',
      emptyValue:
        "`{{name}}` sets `sysparm_name` to a missing, empty, or null method. Pass a non-empty Script Include method name.",
      invalidValue:
        "`{{name}}` sets `sysparm_name` to a non-string value. Pass a non-empty Script Include method name.",
      badPrefix:
        "GlideAjax parameter `{{param}}` must start with `sysparm_`. Use `sysparm_{{param}}`.",
      afterTerminal:
        "`addParam()` after `getXML` / `getXMLAnswer` / `getXMLWait` does not affect that request. Move `addParam()` before the request method.",
    },
  },
  createOnce(context) {
    return {
      ...
      Program(node) {
        ...
        for (const finding of findGlideAjaxParamIssues(node as ESTree.Node, analysis, file)) {
          context.report({
            node: finding.node,
