...
import { definitelySkipsDoWhileTest, truthyPathRequiredCursorIds } from "./cursor-condition.js";
...
export interface RetainedElementFinding {
  node: ESTree.Node;
  name: string;
}
...
function isExtracted(node: unknown, analysis: ProvenanceQuery): boolean {
  ...
  return (
    property === "toString" ||
    (property !== null && analysis.glide.byKind.GlideRecord.valueExtractors.has(property))
  );
}

function objectIdOfCursor(analysis: ProvenanceQuery, node: unknown): number | null {
  return provenReceiver(analysis, node, "GlideRecord")?.objectId ?? null;
}
...
function isCursorAdvanceCall(
  ...
  const property = staticPropertyName(call.callee);
  if (!property || !analysis.glide.byKind.GlideRecord.cursorAdvancers.has(property)) return null;
...
/**
 * GlideElement values pushed into a collection while a cursor loop is still
 * advancing. The element follows the cursor, so the collection ends up holding
 * repeats of the last row.
 */
export function findRetainedElements(
  program: ESTree.Node,
  analysis: ProvenanceQuery,
  authority: PlatformMethodAuthorityFacts,
